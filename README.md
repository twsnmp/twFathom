...
- **データ自動判別・パース**: 入力されたデータ形式を判別し、環境データ、トラフィックデータ、またはシステムメトリクス（CPU/メモリ等）に自動マッピング。
- **インタラクティブな推移グラフ**: スムーズでズーム可能な美しいグラフ表示により、時系列データの詳細な変化をキャッチ。
- **時系列異常検知 (Hotelling's T^2)**: 各種メトリクスから自動で異常度を算出。通常とは異なる変化パターンを日本語の検知メッセージとステータスで表示。
- **スマートなダッシュボード管理**: 起動した複数ウィンドウを自動で格子状に整列（Auto-Arrange）。ウィンドウ位置・サイズ（枠なし状態含む）、グラフ設定を自動保存・復元。
...
### 💻 システム (System) データ
- **CPU/メモリ/ディスク** (CPU, Memory, Disk)
- **プロセス/ロード** (Process, Load)
- **送受信速度** (Tx Speed, Rx Speed)
...
3. **ダッシュボードでの可視化**:
   ソースリストからダッシュボードを開くと、個別の分析ウィンドウ（初期サイズ 640x480）が開きます。複数のダッシュボードウィンドウがある場合、**「自動整列 (Auto-Arrange)」**機能で格子状に綺麗に配置できます。ウィンドウの位置、サイズ（枠なし状態含む）、表示期間、グラフ設定は次回起動時もそのまま復元されます。データソースの削除ボタンは分かりやすく赤色で強調されています。
