...
### Linux (AppImage)
最新のリリースから `.AppImage` パッケージをダウンロードして実行してください。
- **推奨環境**: Ubuntu 22.04 / 24.04 などの主要な Linux ディストリビューション (GNOME / KDE などのデスクトップ環境推奨)
- **起動方法**:
  1. ダウンロードした `.AppImage` ファイルに実行権限を付与します：
     ```bash
     chmod +x twFathom-Linux.AppImage
     ```
  2. ファイルをダブルクリックするか、ターミナルから以下のように実行します：
     ```bash
     ./twFathom-Linux.AppImage
     ```
  *注意: ポータブル形式（AppImage）のため、システムへのインストール作業なしにそのまま実行可能です。*
...
- **保存先ディレクトリ**: `~/.twfathom/twfathom.db` (ホームディレクトリ下の `.twfathom` フォルダ内)
  ...
  - **Linux**: `/home/ユーザー名/.twfathom/twfathom.db`
...
- **Linux**:
  リポジトリの main ブランチにプッシュされると、**GitHub Actions** が自動的にビルドを開始し、ポータブルな `.AppImage` パッケージを生成します。
